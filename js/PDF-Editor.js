...
async function loadPDF(file, pdfNumber, password = null) {
    ...
    try {
        ...
        // === ステップ3.5: オーナーパスワード暗号化の検出と解除 ===
        // pdf.jsで開けても、pdf-libではページコピーできない暗号化（オーナーパスワード）を検出
        if (!needsDecryption) {
            try {
                await PDFLib.PDFDocument.load(await fileToLoad.arrayBuffer());
            } catch (pdfLibError) {
                if (pdfLibError.message && pdfLibError.message.includes('encrypted')) {
                    console.log('🔒 オーナーパスワード暗号化を検出。qpdfで解除します。');
                    loadingMessage.textContent = currentLanguage === 'ja'
                        ? '暗号化を解除中...'
                        : 'Removing encryption...';
                    try {
                        const decryptedFile = await decryptPDFWithQpdf(fileToLoad, '');
                        fileToLoad = decryptedFile;
                        console.log('✅ オーナーパスワード暗号化の解除に成功しました');
                    } catch (decryptError) {
                        console.warn('⚠️ qpdfでの暗号化解除に失敗。画像フォールバックで対応します:', decryptError);
                    }
                }
            }
        }
...
function handleDragOver(e) {
    ...
    if (targetContainer) {
        // PDF未読み込みのペインへのドラッグを無視
        if (draggedElement && draggedElement.parentElement !== targetContainer) {
            const targetPdfNum = targetContainer.id.includes('1') ? 1 : 2;
            if (!pdfDocuments[targetPdfNum - 1]) {
                if (currentDropZone && currentDropZone.parentElement) {
                    currentDropZone.remove();
                    currentDropZone = null;
...
async function handleDrop(e) {
    ...
    if (isSameContainer) {
        ...
        // ★★★ PDFが未読み込みのペインへのコピー防止（二重チェック） ★★★
        if (!pdfDocuments[targetPdfNum - 1]) {
            showStatus(currentLanguage === 'ja' ? 'コピー先にPDFが読み込まれていません' : 'No PDF loaded in the target pane', 'error', 3000);
            return;
        }
...
// qpdfサーバー側でPDFを組み立てる関数
async function assemblePDFWithQpdf(pages, pdfNumber) {
    const pageList = [];
    const usedPdfNums = new Set();
    ...
    for (const pageElement of pages) {
        ...
        pageList.push({ pdf: originalPdfNum, page: originalPageNum });
        usedPdfNums.add(originalPdfNum);
    }
    ...
    for (const pdfNum of usedPdfNums) {
        const sourcePdfData = pdfDocuments[pdfNum - 1];
        if (!sourcePdfData) continue;
        formData.append(`pdf${formKeyNum}`, sourcePdfData.file);
        pdfMapping[pdfNum] = formKeyNum;
        formKeyNum++;
    }

    const remappedPageList = pageList.map(p => ({
        pdf: pdfMapping[p.pdf],
        page: p.page
    }));
    formData.append('pageList', JSON.stringify(remappedPageList));
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.upload.onprogress = (e) => {
            if (e.lengthComputable) {
                ...
                barFill.style.width = `${uploadPercent}%`;
                text.textContent = `${uploadPercent}%`;
                details.textContent = getTranslation('uploadingToServer');
            }
        };
        ...
        xhr.upload.onload = () => {
            ...
            text.textContent = '90%';
            details.textContent = getTranslation('assemblingOnServer');
        };
        ...
        xhr.onload = () => {
            ...
            try {
                ...
                if (!result.success) {
                    reject(new Error(result.message || 'Failed to assemble PDF'));
                    return;
                }
...
// pdf-libによるクライアント側PDF組み立て（フォールバック）
async function assemblePDFWithPdfLib(pages, updateProgressFn) {
    const newPdfDoc = await PDFLib.PDFDocument.create();
    let processedCount = 0;

    for (const pageElement of pages) {
        if (pdfGenerationCancelled) return null;
        ...
        const pdfDataToLoad = await sourcePdfData.file.arrayBuffer();
        ...
        processedCount++;
        updateProgressFn(processedCount, pages.length);
    }
...
const translations = {
    ja: {
        ...
        langLabel: '日本語',
        // パスワード関連
        uploadingToServer: 'サーバーに送信中...',
        assemblingOnServer: 'PDFを組み立て中...',
    ...
    en: {
        ...
        assemblingOnServer: 'Assembling PDF...',

...
/**
 * PDFがパスワードで保護されているかチェックする関数
 * 
 * この関数は、PDFファイルを読み込もうとして、パスワードが
 * 必要かどうかを判定します。
 * 
 * @param {ArrayBuffer} arrayBuffer - PDFファイルのバイナリデータ
 * @returns {Promise<Object>} チェック結果 { isPasswordProtected: boolean, error: Object }
 */
async function checkIfPasswordProtected(arrayBuffer) {
    try {
        // PDFの読み込みを試みる
        // パスワードが必要な場合は、PasswordExceptionエラーが発生します
        const loadingTask = pdfjsLib.getDocument({ data: arrayBuffer });
        await loadingTask.promise;
        
        // エラーが発生しなければ、パスワード保護されていない
        return {
...
// この関数群は、グローバルスコープで定義されているため、
// PDF-Editor.js内の他の関数から自由に呼び出すことができます
